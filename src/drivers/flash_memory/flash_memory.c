...
#include "board_defs.h"
    ...
    static unsigned char iap_vet[FSL_FEATURE_SYSCON_FLASH_PAGE_SIZE_BYTES];
    ...
    unsigned char iap_Reset()
{
    ...
    #ifdef SysEepromSize
        #define EEPROM_FLASH_SIZE SysEepromSize
    #else
        #define EEPROM_FLASH_SIZE 128
    #endif

    #ifdef SysEepromADDR
        #define eeprom_flash_sector        (SysEepromADDR/FSL_FEATURE_SYSCON_FLASH_SECTOR_SIZE_BYTES)
    #else
        #define eeprom_flash_sector        ((FSL_FEATURE_SYSCON_FLASH_SIZE_BYTES-EEPROM_FLASH_SIZE)/FSL_FEATURE_SYSCON_FLASH_SECTOR_SIZE_BYTES)
    #endif

    #ifdef SysEepromADDR
        #define eeprom_flash_page          (SysEepromADDR/FSL_FEATURE_SYSCON_FLASH_PAGE_SIZE_BYTES)
    #else
        #define eeprom_flash_page          ((FSL_FEATURE_SYSCON_FLASH_SIZE_BYTES-EEPROM_FLASH_SIZE)/FSL_FEATURE_SYSCON_FLASH_PAGE_SIZE_BYTES)
    #endif

    #define eeprom_flash_end_sector    (FSL_FEATURE_SYSCON_FLASH_PAGE_SIZE_BYTES/FSL_FEATURE_SYSCON_FLASH_SECTOR_SIZE_BYTES)
    #define eeprom_flash_end_page      (FSL_FEATURE_SYSCON_FLASH_PAGE_SIZE_BYTES/FSL_FEATURE_SYSCON_FLASH_PAGE_SIZE_BYTES)
    #define eeprom_flash_page_num      (EEPROM_FLASH_SIZE/FSL_FEATURE_SYSCON_FLASH_PAGE_SIZE_BYTES)
    #define eeprom_flash_first_addr    (eeprom_flash_page*FSL_FEATURE_SYSCON_FLASH_PAGE_SIZE_BYTES)

    unsigned char eeprom_on = 0;

    //vetor temporario para alocação dos valores que devem ser salvos na eeprom
    static unsigned char eeprom_flash_Vet[EEPROM_FLASH_SIZE];
    /**
    ...
    void eeprom_Begin()
    {
        if(eeprom_on)  
            return;
        for (int i = 0; i < EEPROM_FLASH_SIZE; i++)
            eeprom_flash_Vet[i] = eeprom_ReadChar(i);
        eeprom_on = 1;
    }
    /**
     * @brief lê o valor presente no endereço de memória 
    ...
    {
        return *((unsigned char*)(addr + eeprom_flash_first_addr));
    }
    /**
     * @brief Lê um vetor na eeprom
     * 
    ...
    {
        memcpy(vet, (unsigned char*)(addrInit + eeprom_flash_first_addr),size);
        return FlashOK;
    }
    /**
     * @brief Lê um valor do tipo uInt16 no endereço enviado
    ...
    {
        return *((unsigned short*)(addr + eeprom_flash_first_addr));
    }
    /**
     * @brief Lê um valor do tipo uInt32 no endereço indicado
     * 
    ...
    {
        return *((unsigned int*)(addr + eeprom_flash_first_addr));
    }
    /**
     * @brief Escreve um vetor na memoria temporária
     * 
    ...
    {
        if (EEPROM_FLASH_SIZE < (addrInit+size))
            return FlashFail;
        for (int i = 0; i < size; i++)
            eeprom_flash_Vet[i+addrInit] = vet[i];
        return FlashOK;
    }
    /**
     * @brief Escreve unsigned char no vetor temporário
    ...
    {
        if (addr < EEPROM_FLASH_SIZE)
        {
            eeprom_flash_Vet[addr] = value;
            return FlashOK;
        }
        return FlashFail;
    }
    ...
    {
        if((addr+1) < EEPROM_FLASH_SIZE)
        {
            for (unsigned char i = 0; i < sizeof(unsigned short); i++)
                eeprom_WriteChar((unsigned char) (value>>(8*i)), (addr+i));
            return FlashOK;
        }
        return FlashFail;
    }
    ...
    {
        unsigned char test = 0;
        for (unsigned short i = 0; i < EEPROM_FLASH_SIZE; i++) //verifica se tem algum valor diferente para não desperdiçar
            if (eeprom_flash_Vet[i] != eeprom_ReadChar(i))        //o numero limitado de escritas que são permitidos na memória
            {
                test = 1;
                break;
            }
        if (test == 0)
            return FlashOK;
        //Prepara o setor para escrita (1024 bytes)
        IAP_PrepareSectorForWrite(eeprom_flash_sector, eeprom_flash_end_sector);
        //Limpa as paginas preparadas para escrita (64 bytes * n)
        //IAP_EraseSector(eeprom_flash_sector, FLASH_SIZE_sectors, SystemCoreClock);
        IAP_ErasePage(eeprom_flash_page, eeprom_flash_end_page, SystemCoreClock);
        for (int i = 0; i < eeprom_flash_page_num; i++)
        {
            IAP_PrepareSectorForWrite(eeprom_flash_sector, eeprom_flash_end_sector);
            IAP_CopyRamToFlash(eeprom_flash_first_addr + (FSL_FEATURE_SYSCON_FLASH_PAGE_SIZE_BYTES*i), 
                                (long unsigned int *)(&eeprom_flash_Vet[0] + (FSL_FEATURE_SYSCON_FLASH_PAGE_SIZE_BYTES*i)),
                                FSL_FEATURE_SYSCON_FLASH_PAGE_SIZE_BYTES, SystemCoreClock);
        }
        return FlashOK;
    }
...
{
    for (size_t i = 0; i < EEPROM_FLASH_SIZE; i++)
        eeprom_flash_Vet[i] = 0xFF;
    eeprom_Commit();
    return FlashOK;
}
#endif